...
def get_optional_service(interface: Type[T]) -> Optional[T]:
    """Get an optional service from the global container"""
    return container.get_optional(interface)

@lru_cache(maxsize=128)
def cached_get_service(interface: Type[T]) -> T:
    """Cached version of get_service for better performance"""
    return container.get(interface)