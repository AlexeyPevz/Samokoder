...
class AnthropicClient(BaseLLMClient):
    ...
    async def _make_request(
        ...
        async def single_attempt() -> Tuple[str, int, int]:
            ...
            if json_mode:
                # Anthropic doesn't have a direct JSON mode, but we can add instructions
                # to the system message to encourage JSON responses
                json_instruction = (
                    "Please format your response as valid JSON. "
                    "Make sure to escape any special characters properly. "
                    "Enclose your entire response in a single JSON object."
                )
                system_message = convo.system_message
                if system_message:
                    completion_kwargs["system"] = f"{system_message}\n\n{json_instruction}"
                else:
                    completion_kwargs["system"] = json_instruction
