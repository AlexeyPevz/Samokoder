...
class YAMLParser:
    ...
    def __call__(self, text: str) -> Union[BaseModel, dict, None]:
        self.original_response = text.strip()
        ...
        try:
            ...
            if self.strict:
                raise ValueError(f"YAML is not valid: {e}") from e
            else:
                return None
        ...
        try:
            ...
            errtxt = JSONParser.errors_to_markdown(err.errors())
            raise ValueError(f"Invalid YAML format:\n{errtxt}") from err
        except Exception as err:
            raise ValueError(f"Error parsing YAML: {err}") from err
