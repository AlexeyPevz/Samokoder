...
from samokoder.core.agents.mixins import FileDiffMixin
from samokoder.core.agents.response import AgentResponse, ResponseType
from samokoder.core.config.config import CODE_MONKEY_AGENT_NAME, CODE_REVIEW_AGENT_NAME, DESCRIBE_FILES_AGENT_NAME
from samokoder.core.llm.parser import JSONParser, OptionalCodeBlockParser
...
class CodeMonkey(FileDiffMixin, BaseAgent):
    ...
    async def describe_files(self) -> AgentResponse:
        """
        Describe multiple files using LLM (optimized for parallel execution).
        
        This version processes files concurrently, significantly reducing
        total execution time when describing multiple files.
        """
        ...
        to_describe = {
            ...
            if not file.meta.get("description")
        }

        # Prepare parallel requests for files that need description
        requests = []
        file_mappings = []  # Track which file corresponds to each request
        ...
        for file in self.next_state.files:
            ...
            if content == "":
                # Empty files don't need LLM, set directly
                file.meta = {
                    **file.meta,
            ...
            log.debug(f"Preparing to describe file {file.path}")
        ...
        # Execute all LLM requests in parallel
        if requests:
            log.info(f"Describing {len(requests)} files in parallel")
    ...
    async def run_code_review(self, data: Optional[dict]) -> Union[AgentResponse, dict]:
        ...
        if data is None:
            ...
            return AgentResponse.error(self, "No data provided for code review.")

        # Auto-accept changes under certain conditions to prevent loops or unnecessary reviews.
        is_new_file = not data["old_content"]
        is_unchanged = data["new_content"] == data["old_content"]
        max_attempts_reached = data["attempt"] >= MAX_CODING_ATTEMPTS

        if is_new_file or is_unchanged or max_attempts_reached:
            if max_attempts_reached:
                log.warning(
                    f"Max coding attempts ({MAX_CODING_ATTEMPTS}) reached for {data['path']}. "
                    "Accepting changes without further review."
                )
            # we always auto-accept new files, unchanged files, or if we've tried too many times
            return await self.accept_changes(data["path"], data["old_content"], data["new_content"])
