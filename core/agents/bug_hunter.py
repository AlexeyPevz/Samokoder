...
from samokoder.core.agents.mixins import ChatWithBreakdownMixin, TestSteps
from samokoder.core.agents.response import AgentResponse
from samokoder.core.config.config import CHECK_LOGS_AGENT_NAME, magic_words
...
class BugHunter(ChatWithBreakdownMixin, BaseAgent):
    ...
    async def ask_user_to_test(self, awaiting_bug_reproduction: bool = False, awaiting_user_test: bool = False):
        ...
        if awaiting_bug_reproduction:
            ...
            user_feedback = await self.ask_question(
                "Please add any additional feedback that could help самокодер solve this bug",
                buttons=buttons,
                default="continue",
