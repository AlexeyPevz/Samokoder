...
from samokoder.core.templates.example_project import EXAMPLE_PROJECT_DESCRIPTION
...
class Importer(BaseAgent):
    ...
    async def start_import_process(self):
        ...
        if imported_lines > MAX_PROJECT_LINES:
            await self.send_message(
                "WARNING: Your project ({imported_lines} LOC) is larger than supported and may cause issues in Samokoder."
            )
        ...
        self.next_state.epics = [
            {
                "id": uuid4().hex,
                "name": "Import project",
                "description": "Import an existing project into самокодер",
                "tasks": [],
                "completed": True,
