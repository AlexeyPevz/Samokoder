...
from samokoder.core.agents.mixins import ChatWithBreakdownMixin, IterationPromptMixin, RelevantFilesMixin, TestSteps
from samokoder.core.agents.response import AgentResponse
from samokoder.core.config.config import TROUBLESHOOTER_GET_RUN_COMMAND
...
class Troubleshooter(ChatWithBreakdownMixin, IterationPromptMixin, RelevantFilesMixin, BaseAgent):
    ...
    async def create_iteration(self) -> AgentResponse:
        ...
        if not user_instructions:
            ...
            await self.ui.send_message("Протестируйте приложение, следуя этим шагам:", source=samokoder_source)
    ...
    async def get_user_instructions(self) -> Optional[str]:
        ...
        if len(user_instructions.steps) == 0:
            await self.ui.send_message(
                "Тестирование не требуется для этой задачи, переходим к следующей.", source=samokoder_source
            )
    ...
    async def get_user_feedback(
        ...
        If "is_loop" is True, самокодер застрял в цикле и нужно рассмотреть альтернативные решения.
