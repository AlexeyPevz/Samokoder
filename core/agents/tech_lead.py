...
from samokoder.core.agents.mixins import RelevantFilesMixin
from samokoder.core.agents.response import AgentResponse
from samokoder.core.config.config import TECH_LEAD_EPIC_BREAKDOWN, TECH_LEAD_PLANNING
from samokoder.core.db.models import Complexity
from samokoder.core.db.models.project_state import TaskStatus
...
class TechLead(RelevantFilesMixin, BaseAgent):
    ...
    async def ask_for_new_feature(self) -> AgentResponse:
        ...
        if response.button == "end" or response.cancelled or not response.text:
            await self.ui.send_message("Спасибо, что используете самокодер!", source=samokoder_source)
