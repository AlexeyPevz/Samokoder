...
WATCHER_IDLE_INTERVAL = 0.1
BUSY_WAIT_INTERVAL = 0.1
MAX_COMMAND_TIMEOUT = 300.0  # 5 minutes
...
class ProcessManager:
    ...
    async def run_command(
        ...
        *,
        ...
        use_container: bool = True, # Flag to control execution context
    ) -> tuple[Optional[int], str, str]:
        """
        Run command and wait for it to finish.

        :param use_container: If True and a container is configured, run inside the container.
        """
        if self.container and use_container:
            log.debug(f"Running command in container: {cmd}")
            ...
            if self.output_handler and (stdout_str or stderr_str) and show_output:
                await self.output_handler(stdout_str, stderr_str)
    ...
    async def start_hot_reload_process(self, cmd: str, watch_paths: list[str], ui_callback=None):
        ...
        #     await self.ui.send_message(f"File change detected in {event.src_path}. Restarting process...", source="system")
        #     await self.terminate_process(process.id)
        #     new_process = await self.start_process(cmd, bg=True)
        #     process = new_process # Update the process reference
        #     await self.ui.send_message(f"Restarted process `{cmd}` (PID: {new_process.pid}).", source="system")
        #
