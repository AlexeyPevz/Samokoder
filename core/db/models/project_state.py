...
from samokoder.core.db.models import Base, FileContent
...
class ProjectState(Base):
    ...
    prev_state_id: Mapped[Optional[GUID]] = mapped_column(ForeignKey("project_states.id", ondelete="CASCADE"))
    ...
    # Normalized relationships (alternatives to JSONB fields)
    epic_models: Mapped[list["Epic"]] = relationship(
        back_populates="project_state",
        lazy="selectin",
        cascade="all,delete-orphan",
        order_by="Epic.epic_index"
    )
    task_models: Mapped[list["Task"]] = relationship(
        back_populates="project_state",
        lazy="selectin",
        cascade="all,delete-orphan",
        order_by="Task.task_index"
    )
    step_models: Mapped[list["Step"]] = relationship(
        back_populates="project_state",
        lazy="selectin",
        cascade="all,delete-orphan",
        order_by="Step.step_index"
    )
    iteration_models: Mapped[list["Iteration"]] = relationship(
        back_populates="project_state",
        lazy="selectin",
        cascade="all,delete-orphan",
    ...
    def unfinished_steps(self) -> list[dict]:
        ...
        """
        return [step.to_dict() for step in self.step_models if not step.completed]
    ...
    def unfinished_iterations(self) -> list[dict]:
        ...
        return [
            iteration.to_dict()
            for iteration in self.iteration_models
            if iteration.status not in (None, IterationStatus.DONE)
        ]
    ...
    def unfinished_tasks(self) -> list[dict]:
        ...
        """
        return [task.to_dict() for task in self.task_models if task.status != TaskStatus.DONE]
    ...
    def unfinished_epics(self) -> list[dict]:
        ...
        """
        return [epic.to_dict() for epic in self.epic_models if not epic.completed]
    ...
    def create_initial_state(branch: "Branch") -> "ProjectState":
        ...
        """
        from samokoder.core.db.models import Specification
    ...
    def get_file_content_by_path(self, path: str) -> Union[FileContent, str]:
        ...
        return file.content.content if file and file.content else ""
    ...
    def save_file(self, path: str, content: "FileContent", external: bool = False) -> "File":
        ...
        If the file was created by Samokoder (not externally by user or template import),
        ...
        """
        from samokoder.core.db.models import File
        ...
        file = self.get_file_by_path(path)
        if file:
            original_content = file.content.content if file.content else ""
    ...
    async def delete_after(self, session: AsyncSession):
        """
