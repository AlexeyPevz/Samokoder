...
class LegacyDatabaseImporter:
    ...
    async def load_legacy_database(self):
        async with aiosqlite.connect(self.dbpath) as conn:
            ...
            if not is_valid:
                raise ValueError(f"Database {self.dbpath} doesn't look like a Samokoder database")
