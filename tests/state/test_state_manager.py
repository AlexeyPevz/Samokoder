...
from samokoder.core.config.config import FileSystemConfig
...
async def test_create_project(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_load_project(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_delete_project(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_load_project_branch(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_load_nonexistent_step(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_load_specific_step(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_commit(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
    sm = StateManager(testmanager)
    project = await sm.create_project("test", user_id=test_user.id)
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_save_file(mock_get_config, testmanager, test_user):
    mock_get_config.return_value.fs.type = "memory"
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_importing_changed_files_to_db(mock_get_config, tmpdir, testmanager, test_user):
    ...
    async with testmanager as session:
        ...
        os.remove(os.path.join(project_root, "file1.txt"))  # Remove the first file
        with open(os.path.join(project_root, "file2.txt"), "a") as f:
            f.write("modified")  # Change the second file

        await sm.import_files()
        await sm.commit()

        assert not os.path.exists(os.path.join(project_root, "file1.txt"))
        assert os.path.exists(os.path.join(project_root, "file2.txt"))
        assert os.path.exists(os.path.join(project_root, "file3.txt"))
...
@pytest.mark.asyncio
@patch("core.state.state_manager.get_config")
async def test_restoring_files_from_db(mock_get_config, tmpdir, testmanager, test_user):
    mock_get_config.return_value.fs = FileSystemConfig(workspace_root=str(tmpdir))
    sm = StateManager(testmanager)
    project = await sm.create_project("test1", user_id=test_user.id)
    ...
    async with testmanager as session:
        ...
        assert os.path.exists(os.path.join(project_root, "file1.txt"))
        assert os.path.exists(os.path.join(project_root, "file2.txt"))
        assert os.path.exists(os.path.join(project_root, "file3.txt"))

        assert open(os.path.join(project_root, "file1.txt")).read() == "this is the content 1"
        assert open(os.path.join(project_root, "file2.txt")).read() == "this is the content 2"
        assert open(os.path.join(project_root, "file3.txt")).read() == "this is the content 3"
