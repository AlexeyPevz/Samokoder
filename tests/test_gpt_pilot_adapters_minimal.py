#!/usr/bin/env python3
"""
Минимальные тесты для GPT Pilot адаптеров - только существующие методы
"""
...
class TestSamokoderGPTPilotSimpleAdapter:
    """Минимальные тесты для SamokoderGPTPilotSimpleAdapter"""
    ...
    def test_get_file_content_not_found(self):
        """Тест получения содержимого файла - файл не найден"""
        content = self.adapter.get_file_content("nonexistent.py")
        
        assert content == "File not found: nonexistent.py"
    
    def test_get_file_content_found(self):
        """Тест получения содержимого файла - файл найден"""
        with patch.object(self.adapter, 'workspace') as mock_workspace:
            mock_file = Mock()
            mock_file.exists.return_value = True
            mock_file.read_text.return_value = "print('hello')"
            mock_workspace.__truediv__ = Mock(return_value=mock_file)
            
            content = self.adapter.get_file_content("test.py")
...
class TestSamokoderGPTPilotRealAdapter:
    """Минимальные тесты для SamokoderGPTPilotRealAdapter"""
    ...
    def test_create_zip_export(self):
        """Тест создания ZIP экспорта"""
        with patch('backend.services.gpt_pilot_real_adapter.shutil.make_archive') as mock_make_archive:
            mock_make_archive.return_value = "/tmp/test.zip"
            
            result = self.adapter.create_zip_export()
