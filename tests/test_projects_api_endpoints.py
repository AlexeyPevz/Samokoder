...
class TestProjectsAPICreate:
    ...
    @pytest.mark.asyncio
    async def test_create_project_workspace_error(self, mock_current_user, mock_rate_limit, valid_project_data):
        """Test project creation with workspace creation error"""
        from backend.api.projects import create_project
        ...
        with patch('backend.api.projects.validate_project_name', return_value=True), \
            ...
            mock_path_obj = Mock()
            mock_path.return_value = mock_path_obj
            mock_path_obj.resolve.side_effect = Exception("Path error")
            ...
            assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert "Failed to create workspace directory" in str(exc_info.value.detail)
...
class TestProjectsAPIList:
    ...
    async def test_list_projects_success(self, mock_current_user, mock_rate_limit):
        ...
        mock_projects_data = [
            ...
            },
            {
                ...
                "workspace_path": "workspaces/user123/project2",
    ...
    @pytest.mark.asyncio
    async def test_list_projects_with_filters(self, mock_current_user, mock_rate_limit):
        ...
        with patch('backend.api.projects.execute_supabase_operation', return_value=Mock(data=mock_projects_data)) as mock_supabase:
            await list_projects(mock_current_user, limit=5, offset=10, status="active", search="test", rate_limit=mock_rate_limit)
    ...
    async def test_list_projects_database_error(self, mock_current_user, mock_rate_limit):
        ...
        with patch('backend.api.projects.execute_supabase_operation', side_effect=Exception("Database error")):
            with pytest.raises(HTTPException) as exc_info:
                await list_projects(mock_current_user, limit=10, offset=0, status=None, search=None, rate_limit=mock_rate_limit)
            
            assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert "Failed to list projects" in str(exc_info.value.detail)
...
class TestProjectsAPIGet:
    ...
    async def test_get_project_success(self, mock_current_user, mock_rate_limit):
        ...
        with patch('backend.api.projects.execute_supabase_operation', return_value=Mock(data=[mock_project_data])):
            ...
            assert result.ai_config == {"model": "gpt-4"}
    ...
    async def test_get_project_database_error(self, mock_current_user, mock_rate_limit):
        ...
        with patch('backend.api.projects.execute_supabase_operation', side_effect=Exception("Database error")):
            with pytest.raises(HTTPException) as exc_info:
                await get_project("project123", mock_current_user, mock_rate_limit)
            
            assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert "Failed to get project" in str(exc_info.value.detail)
...
class TestProjectsAPIUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_project_update_failed(self, mock_current_user, mock_rate_limit, update_data):
        """Test update project when update fails"""
        from backend.api.projects import update_project
        ...
        with patch('backend.api.projects.execute_supabase_operation') as mock_supabase:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await update_project("project123", request, mock_current_user, mock_rate_limit)
            
            assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert "Failed to update project" in str(exc_info.value.detail)
...
class TestProjectsAPIDelete:
    ...
    async def test_delete_project_success(self, mock_current_user, mock_rate_limit):
        ...
        with patch('backend.api.projects.execute_supabase_operation') as mock_supabase:
            ...
            mock_supabase.return_value = Mock(data=[{"id": "project123", "is_active": False}])
    ...
    @pytest.mark.asyncio
    async def test_delete_project_deletion_failed(self, mock_current_user, mock_rate_limit):
        """Test delete project when deletion fails"""
        from backend.api.projects import delete_project
        ...
        with patch('backend.api.projects.execute_supabase_operation') as mock_supabase:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await delete_project("project123", mock_current_user, mock_rate_limit)
            
            assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert "Failed to delete project" in str(exc_info.value.detail)
