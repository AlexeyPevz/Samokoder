#!/usr/bin/env python3
"""
Финальные тесты для GPT Pilot Wrapper v2
"""
...
class TestSamokoderGPTPilotFinal:
    """Финальные тесты для SamokoderGPTPilot"""
    ...
    @pytest.mark.asyncio
    async def test_update_project_description_error(self):
        """Тест обновления описания проекта с ошибкой"""
        mock_adapter = Mock()
        mock_project = Mock()
        mock_adapter.project = mock_project
        mock_project.name = "old_name"
        
        # Симулируем ошибку при обращении к атрибуту
        def side_effect(value):
            raise Exception("Name error")
        
        mock_project.__setattr__ = side_effect
    ...
    @pytest.mark.asyncio
    async def test_get_agent_status_error(self):
        """Тест получения статуса агентов с ошибкой"""
        mock_adapter = Mock()
        # Симулируем ошибку при обращении к атрибуту initialized
        def side_effect():
            raise Exception("Initialized error")
        
        mock_adapter.__getattr__ = side_effect
