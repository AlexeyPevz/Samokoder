...
class TestFileUploadAPI:
    ...
    def test_upload_file_success(self, mock_error_context, mock_scan, mock_save, mock_validate, 
        ...
        mock_error_context.return_value = {"request_id": "req123"}
    ...
    @patch('backend.api.file_upload.validate_path_traversal')
    @patch('backend.api.file_upload.create_error_context')
    def test_delete_file_invalid_path(self, mock_error_context, mock_path_validation, 
                                    mock_current_user):
        ...
        # Выполнение запроса
        response = self.client.delete("/delete/../../../etc/passwd")
        
        # Проверки
        assert response.status_code == 400
        response_data = response.json()
        assert "invalid path" in response_data["detail"].lower()
    
    @patch('backend.api.file_upload.get_current_user')
    @patch('backend.api.file_upload.validate_path_traversal')
    @patch('backend.api.file_upload.create_error_context')
    def test_get_file_info_invalid_path(self, mock_error_context, mock_path_validation, 
                                      mock_current_user):
        ...
        # Выполнение запроса
        response = self.client.get("/info/../../../etc/passwd")
