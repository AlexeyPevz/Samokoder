#!/usr/bin/env python3
"""
Тесты для GPT Pilot Wrapper v2
"""
...
class TestSamokoderGPTPilot:
    """Тесты для SamokoderGPTPilot"""
    ...
    async def test_update_project_description_error(self):
        ...
        mock_adapter = Mock()
        mock_adapter.project = Mock()
        mock_adapter.project.name = "old_name"
        mock_adapter.project.name = PropertyError("Name error")
    ...
    async def test_get_agent_status_error(self):
        ...
        mock_adapter = Mock()
        mock_adapter.initialized = True
        mock_adapter.initialized = PropertyError("Initialized error")
