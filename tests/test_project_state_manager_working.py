#!/usr/bin/env python3
"""
Рабочие тесты для Project State Manager
"""
...
class TestProjectStateManagerWorking:
    """Рабочие тесты для ProjectStateManager"""
    ...
    async def test_load_project_from_db_success(self):
        ...
        with patch('backend.services.project_state_manager.execute_supabase_operation') as mock_supabase, \
            ...
            mock_response = Mock()
            mock_response.data = [{"id": project_id, "user_id": user_id, "api_key_encrypted": "encrypted_key"}]
            ...
            # Проверяем что проект был добавлен в менеджер
            assert project_id in self.manager._projects
            assert self.manager._projects[project_id].pilot_wrapper == mock_pilot_wrapper
...
class TestProjectStateManagerGlobalFunctionsWorking:
    """Рабочие тесты для глобальных функций Project State Manager"""
