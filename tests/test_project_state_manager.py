#!/usr/bin/env python3
"""
Тесты для Project State Manager
"""
...
class TestProjectStateManager:
    ...
    async def test_add_project_success(self):
        ...
        with patch.object(self.manager, '_remove_oldest_project_if_needed') as mock_remove:
    ...
    async def test_add_project_max_projects_reached(self):
        ...
        with patch.object(self.manager, '_remove_oldest_project') as mock_remove_oldest:
            result = await self.manager.add_project("new_project", "new_user", Mock())
            
            assert result is True
            mock_remove_oldest.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_load_project_from_db_success(self):
        """Тест загрузки проекта из базы данных - успех"""
        project_id = "project123"
        user_id = "user123"
        
        mock_pilot_wrapper = Mock()
        
        with patch('backend.services.project_state_manager.execute_supabase_operation') as mock_supabase, \
             patch('backend.services.project_state_manager.SamokoderGPTPilot') as mock_pilot_class:
            
            mock_supabase.return_value = Mock(data=[{"id": project_id, "user_id": user_id}])
            mock_pilot_class.return_value = mock_pilot_wrapper
            
            result = await self.manager._load_project_from_db(project_id, user_id)
            
            assert result == mock_pilot_wrapper
    ...
    async def test_get_stats(self):
        ...
        assert stats["initialized"] is False  # Не инициализирован в тесте
    
    @pytest.mark.asyncio
    async def test_close(self):
        """Тест закрытия менеджера"""
        # Инициализируем менеджер
        mock_task = Mock()
        self.manager._cleanup_task = mock_task
        self.manager._initialized = True
...
class TestProjectStateManagerGlobalFunctions:
    """Тесты для глобальных функций Project State Manager"""
