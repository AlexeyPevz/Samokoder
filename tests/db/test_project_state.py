...
from samokoder.core.db.models import Branch, File, FileContent, Project, ProjectState
...
@pytest.mark.asyncio
async def test_get_by_id(testdb, test_user):
...
@pytest.mark.asyncio
async def test_get_by_id_preloads_branch_project_files(testdb, test_user):
...
@pytest.mark.asyncio
async def test_create_next_state_clones_files(testdb, test_user):
    f = File(path="test.txt", content=FileContent(id="test", content="hello world"))
...
@pytest.mark.asyncio
async def test_create_next_deep_copies_fields(testdb, test_user):
    state = create_project_state(user=test_user)
    testdb.add(state)
...
@pytest.mark.asyncio
async def test_deleting_state_removes_child_objects(testdb, test_user):
    ...
    await state.delete_after(testdb)
...
@pytest.mark.asyncio
async def test_completing_unfinished_steps(testdb, test_user):
    state = create_project_state(user=test_user)
    state.steps = [
        {
            "id": "abc",
...
@pytest.mark.asyncio
async def test_completing_unfinished_iterations(testdb, test_user):
    state = create_project_state(user=test_user)
    state.iterations = [
        {
            "id": "abc",
...
@pytest.mark.asyncio
async def test_completing_unfinished_tasks(testdb, test_user):
    state = create_project_state(user=test_user)
    state.tasks = [
        {
            "id": "abc",
...
@pytest.mark.asyncio
async def test_completing_unfinished_epics(testdb, test_user):
    state = create_project_state(user=test_user)
    state.epics = [
        {
            "id": "abc",
