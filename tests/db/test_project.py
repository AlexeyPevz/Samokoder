...
from samokoder.core.db.models import Branch, Project
...
async def test_get_by_id(testdb, test_user):
    project = Project(name="test", user_id=test_user.id)
    testdb.add(project)
    await testdb.commit()
...
@pytest.mark.asyncio
async def test_delete_by_id(testdb, test_user):
    project = Project(name="test", user_id=test_user.id)
    testdb.add(project)
    await testdb.commit()
...
@pytest.mark.asyncio
async def test_get_branch_no_match(testdb, test_user):
    project = Project(name="test", user_id=test_user.id)
    testdb.add(project)
    await testdb.commit()
...
@pytest.mark.asyncio
async def test_get_branch(testdb, test_user):
    project = Project(name="test", user_id=test_user.id)
...
@pytest.mark.asyncio
async def test_get_branch_no_session(test_user):
    project = Project(name="test", user_id=test_user.id)
...
@pytest.mark.asyncio
async def test_get_all_projects(testdb, test_user):
    state1 = create_project_state(user=test_user)
    state2 = create_project_state(user=test_user)
    testdb.add(state1.branch.project)
    testdb.add(state2.branch.project)
    await testdb.flush()
...
@pytest.mark.asyncio
async def test_default_folder_name(testdb, test_user):
