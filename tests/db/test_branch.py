...
@pytest.mark.asyncio
async def test_get_last_state_no_steps(testdb, test_user):
    project = Project(name="test", user_id=test_user.id)
    branch = Branch(project=project)
    testdb.add(project)
    await testdb.commit()
...
@pytest.mark.asyncio
async def test_get_last_state(testdb, test_user):
...
@pytest.mark.asyncio
async def test_get_last_state_no_session(test_user):
    project = Project(name="test", user_id=test_user.id)
    branch = Branch(project=project)
