"""
Simple tests for API Keys API endpoints - Fixed version with correct model fields
"""
import pytest
from unittest.mock import Mock, AsyncMock, patch
from backend.models.requests import APIKeyCreateRequest
from backend.models.responses import APIKeyResponse, APIKeyListResponse, AIProvider
...
class TestAPIKeysAPIModels:
    ...
    def test_api_key_response_model(self):
        ...
        assert response.key_name == "My OpenAI Key"
    ...
    def test_api_key_list_response_model(self):
        ...
        assert response.keys[0].provider == "openai"
        assert response.keys[1].provider == "anthropic"
    ...
    def test_api_key_create_request_validation(self):
        ...
        assert valid_request.provider == AIProvider.OPENAI
        assert len(valid_request.key_name) > 0
        
        # Test with different provider
        request_anthropic = APIKeyCreateRequest(
            api_key="sk-ant-1234567890abcdef1234567890abcdef",
            provider=AIProvider.ANTHROPIC,
            key_name="Anthropic Key"
        )
        
        assert request_anthropic.provider == AIProvider.ANTHROPIC
...
class TestAPIKeysAPIDataFlow:
    ...
    def test_api_key_data_transformation(self):
        ...
        api_key_response = APIKeyResponse(
            ...
            key_name=raw_key_data["key_name"],
        ...
        assert api_key_response.key_name == "My OpenAI Key"
    ...
    def test_api_key_request_validation(self):
        ...
        # Test request with different provider
        ...
        assert full_request.provider == AIProvider.ANTHROPIC
        assert full_request.key_name == "Anthropic Key"
...
class TestAPIKeysAPIMockData:
    ...
    def test_mock_api_key_creation_data(self):
        ...
        key_record = {
            ...
            "key_name": mock_key_data["key_name"],
        ...
        assert key_record["key_name"] == "My OpenAI Key"
...
class TestAPIKeysAPIEdgeCases:
    ...
    def test_unicode_handling(self):
        ...
        unicode_key_data = {
            "api_key": "sk-1234567890abcdef1234567890abcdef",
            "provider": "openai",
            "key_name": "Ключ API",  # Unicode name
        }
        
        # Test unicode handling
        assert isinstance(unicode_key_data["key_name"], str)
        
        # Should be able to encode/decode
        name_encoded = unicode_key_data["key_name"].encode("utf-8")
        name_decoded = name_encoded.decode("utf-8")
        assert name_decoded == unicode_key_data["key_name"]
